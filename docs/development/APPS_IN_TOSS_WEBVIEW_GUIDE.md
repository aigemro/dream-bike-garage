...
## 13. 앱인토스 콘솔 등록 및 출시 준비 현황

> 기준일: **2026-08-26**

> [!IMPORTANT]
> 앱인토스 콘솔은 **게임 등급분류 신청 직전 단계까지 정보를 작성하고 임시저장한 상태**다.  
> 게임 등급분류 신청, 앱인토스 SDK 적용, 빌드 업로드 및 검토 요청은 아직 진행하지 않았다.

### 13.1 현재 상태 요약

| 구분 | 상태 | 비고 |
|---|---|---|
| 콘솔 기본 정보 | 입력 완료 | 한국어·영어 앱 이름, 부제, 상세 설명, `appName`, 고객문의 정보 |
| 카테고리·검색 정보 | 입력 완료 | 게임 카테고리와 앱 검색 키워드 설정 |
| 노출 이미지 | 등록 준비 | 로고, 다크모드 로고, 썸네일, 세로·가로 스크린샷 |
| 콘솔 저장 상태 | 임시저장 완료 | 등급분류 신청 전 단계까지 작성 |
| 게임 등급분류 | 신청 전 | 최종 게임 내용과 주요 화면 확정 후 신청 |
| 앱인토스 실행환경 | 미적용 | MVP 기능 개발 완료 후 SDK와 WebView 실행환경 적용 |
| 다음 작업 | 진행 예정 | 일반 프로젝트 MVP 개발 완료 및 앱인토스 실행환경 적용 |

### 13.2 콘솔 등록 및 노출 자산 준비 내역
...
- 게임 카테고리 설정
- 앱 검색 키워드 설정
- 아래 규격에 맞춘 노출 이미지 등록 준비

| 자산 | 규격 | 수량 | 현재 상태 |
|---|---:|---:|---|
| 앱 로고 | 600×600 | 1장 | 등록 준비 |
| 다크모드 앱 로고 | 600×600 | 1장 | 등록 준비 |
| 썸네일 | 1932×828 | 1장 | 등록 준비 |
| 세로형 스크린샷 | 636×1048 | 최소 3장 | 등록 준비 |
| 가로형 스크린샷 | 1504×741 | 최소 1장 | 등록 준비 |

### 13.3 게임 등급분류 진행 기준

앱인토스 콘솔에서 게임 등급분류 단계까지 진입했지만, **현재는 등급분류를 신청하지 않는다.**

등급분류를 받은 게임과 실제 앱인토스 출시 게임의 주요 내용·화면이 일치해야 한다. 현재 일반 프로젝트에는 MVP 개발과 앱인토스 출시 대응 작업이 남아 있으므로, 개발 중인 화면과 기능을 기준으로 먼저 신청하면 등급분류 자료와 최종 출시본 사이에 차이가 생길 수 있다.

따라서 **MVP 핵심 흐름이 최종 동작하고 앱인토스용 주요 화면이 확정된 이후** 게임물 등급분류를 신청한다.

등급분류 완료 후 발급·확정된 실제 정보를 기준으로 앱인토스 콘솔의 다음 항목을 작성한다.
...
### 13.4 등급분류 신청 전 완료 조건

다음 핵심 흐름이 출시 후보 빌드에서 처음부터 끝까지 동작하고, 각 단계의 주요 화면이 확정되어야 한다.

> Garage 홈 → 고객 주문 → 부품 수급 → 2-to-1 머지 → 자전거 조립 → 납품 → 코인 보상 → 드림 바이크 성장·수집 → 다음 주문 → 앱 종료 및 재진입 시 진행 상태 복구

기능이나 주요 화면이 변경될 가능성이 남아 있으면 등급분류 신청을 미루고 MVP와 앱인토스 실행환경 검증을 먼저 완료한다.

### 13.5 이후 진행 순서

- [x] 앱인토스 콘솔 기본·노출 정보 입력 및 등급분류 신청 직전까지 임시저장
- [ ] 일반 프로젝트 MVP 기능 개발 완료
- [ ] 앱인토스 SDK 및 실행환경 적용
- [ ] 앱인토스 세로 모바일 화면 대응
- [ ] Safe Area 및 터치 조작 검증
- [ ] 뒤로가기 및 앱 복귀 동작 검증
- [ ] 로컬 저장 및 재진입 상태 복구 검증
- [ ] 오디오 및 게임 피드백 검증
- [ ] 앱인토스 Sandbox 테스트
- [ ] Toss 실기기 테스트
- [ ] 최종 게임 주요화면 확정
...
- [ ] 등급분류 완료 및 증명자료 확보
- [ ] 앱인토스 콘솔 게임 등급정보 입력
- [ ] 등급분류 화면과 앱인토스 게임 화면 비교자료 등록
- [ ] 앱인토스 최종 검토 요청
- [ ] 출시

### 13.6 상태 기록

- **앱인토스 콘솔 등록:** 등급분류 신청 직전까지 작성 및 임시저장 완료
- **다음 출시 준비 작업:** 일반 프로젝트 MVP 개발 완료 및 앱인토스 실행환경 적용
- **게임 등급분류 신청 시점:** MVP 개발과 앱인토스용 주요 화면이 최종 확정된 이후
