...
- 기본 수동 흐름에서는 상자를 개봉한 뒤 보드에 놓기 전에 실제 부품 모양과 배치 가능 여부를 미리 표시한다.
- 택배 선반의 **자동 배치** 체크박스를 켜면 배송 완료 시 Lv.1 부품을 첫 번째 배치 가능한 위치에 바로 넣는다.
- 자동 배치는 위에서 아래, 왼쪽에서 오른쪽 순서로 공간을 찾고 필요한 경우 회전 가능한 방향을 확인한다.
- 공간이 없으면 택배를 도착 상태로 보존하며, 자동 정리·자동 머지·자동 주문은 수행하지 않는다.
- 체크 상태는 MVP 로컬 진행 상태와 함께 저장한다.
...
- 수집용 자전거 구매와 도감
- 진행 상태 저장
- 5일 주기 대회 달력, 보유 대표 자전거로 참가하는 리버사이드 3K 자동 레이스
- 참가비 차감과 순위 상금 정산 저장
