...
import {
  ...
  plotCell,
  cellAt,
  plotLine as rasterLine,
  plotRing as rasterRing,
  plotRect as rasterRect,
  outlineGrid,
  forEachRun,
} from './pixel-raster';
...
// - 레이스 연출을 위해 리그 지오메트리(bikeRig)와, 회전 부품(림·스포크·허브 / 크랭크·페달)을
//   뺀 몸체 그리드(buildBikeGrid의 detach 옵션)를 노출합니다. 이 모듈은 Phaser를 타입으로만 참조하므로
//   그리드 빌드·색 결정 함수는 node(Vitest)에서도 실행됩니다.
...
export type BikePixelRole =
  ...
  | 'bar' | 'stem' | 'grip'
  | 'ink';
type Role = BikePixelRole;
type PaintRole = Exclude<Role, 'ink'>;

// 역할 → 부품 그룹 매핑 (잉크 외곽선은 이웃 픽셀의 그룹을 따라갑니다)
const ROLE_GROUP: Record<PaintRole, BikePartGroup> = {
...
export type BikeCell = { role: Role; group: BikePartGroup };
export type BikeGrid = PixelGrid<BikeCell>;

/**
 * 회전 연출을 위해 몸체에서 떼어 낼 수 있는 부품.
 * spokes=스포크·허브(림은 타이어와 함께 몸체에 남겨 이음새·잉크 침범을 막습니다), crankset=크랭크 암·페달
 */
export type BikeDetachablePart = 'spokes' | 'crankset';
...
// ─── 픽셀 래스터 프리미티브 (역할 기반 래퍼) ───────────────────────────

const GRID_W = 64;
const GRID_H = 40;

function makeGrid(): BikeGrid {
  return makePixelGrid<BikeCell>(GRID_W, GRID_H);
}

// 같은 역할은 같은 셀 객체를 공유합니다(불변). 셀 비교는 role/group 값으로 합니다.
const CELL_OF: Record<PaintRole, BikeCell> = Object.fromEntries(
  (Object.keys(ROLE_GROUP) as PaintRole[]).map((role) => [role, { role, group: ROLE_GROUP[role] }]),
) as Record<PaintRole, BikeCell>;

function plot(grid: BikeGrid, x: number, y: number, role: PaintRole) {
  plotCell(grid, x, y, CELL_OF[role]);
}

function plotLine(grid: BikeGrid, x0: number, y0: number, x1: number, y1: number, role: PaintRole, thickness = 1) {
  rasterLine(grid, x0, y0, x1, y1, CELL_OF[role], thickness);
}

function plotRing(grid: BikeGrid, cx: number, cy: number, radius: number, thickness: number, roleFor: (x: number, y: number) => PaintRole) {
  rasterRing(grid, cx, cy, radius, thickness, (x, y) => CELL_OF[roleFor(x, y)]);
}

function plotRect(grid: BikeGrid, x0: number, y0: number, x1: number, y1: number, role: PaintRole) {
  rasterRect(grid, x0, y0, x1, y1, CELL_OF[role]);
}

// ─── 바퀴·음영·외곽선 공통 처리 ────────────────────────────────────────

export type WheelStyle = { radius: number; tireThickness: number; knobStep?: number; spokeStep: number };
type Detach = { spokes: boolean; crankset: boolean };
const NO_DETACH: Detach = { spokes: false, crankset: false };

function plotWheel(grid: BikeGrid, cx: number, cy: number, style: WheelStyle, detach: Detach) {
  const { radius, tireThickness, knobStep } = style;
  ...
  // 스포크·허브는 레이스 연출에서 회전 레이어로 따로 그리므로 detach 시 생략합니다.
  // 림은 잉크를 두르지 않는 역할이라 안쪽이 비어도 잉크가 끼지 않습니다.
  if (detach.spokes) return;
  plotWheelSpokes(grid, cx, cy, style, 0, 'spoke');
  plotWheelHub(grid, cx, cy);
}

/** 스포크 반지름 범위(허브 바깥 1.6 ~ 림 안쪽 1.4) */
export function spokeSpan(style: WheelStyle): { inner: number; outer: number } {
  return { inner: 1.6, outer: style.radius - style.tireThickness - 1.4 };
}

/**
 * 스포크를 spokeAngle(라디안)만큼 돌려 찍습니다. 잉크를 두르지 않는 1px 디테일입니다.
 * 레이스 라이더 스프라이트가 회전 위상별 프레임을 만들 때 재사용합니다.
 */
export function plotWheelSpokes(grid: BikeGrid, cx: number, cy: number, style: WheelStyle, spokeAngle: number, role: 'spoke' | 'hub' = 'spoke') {
  const span = spokeSpan(style);
  for (let angle = 0; angle < 360; angle += style.spokeStep) {
    const rad = (angle * Math.PI) / 180 + spokeAngle;
    plotLine(
      grid,
      cx + Math.cos(rad) * span.inner, cy + Math.sin(rad) * span.inner,
      cx + Math.cos(rad) * span.outer, cy + Math.sin(rad) * span.outer,
      role, 1,
    );
  }
}
...
/** 허브 십자(5칸) */
export function plotWheelHub(grid: BikeGrid, cx: number, cy: number) {
  plot(grid, cx, cy, 'hub');
...
function applyFrameShading(grid: BikeGrid) {
...
const INK_CELL_OF: Record<BikePartGroup, BikeCell> = {
  frame: { role: 'ink', group: 'frame' },
  wheel: { role: 'ink', group: 'wheel' },
  drivetrain: { role: 'ink', group: 'drivetrain' },
  handlebar: { role: 'ink', group: 'handlebar' },
};

export function applyBikeInkOutline(grid: BikeGrid) {
  outlineGrid(grid, (cell) => cell.role !== 'ink' && OUTLINED_ROLES.has(cell.role), (neighbor) => INK_CELL_OF[neighbor.group]);
}
...
/**
 * 자전거 리그(그리드 좌표, 64×40, 앵커 x=32 / y=바퀴 축).
 * 라이더 착좌·핸들 파지·페달링 좌표와 바퀴 회전 프레임이 이 값에서 파생됩니다.
 */
export type BikeRig = {
  category: BikeCategory;
  gridWidth: number;
  ...
  /** BB(크랭크 중심) */
  bb: [number, number];
  wheel: { radius: number; tireThickness: number; spokeStep: number; knobStep?: number };
  chainringRadius: number;
  /** 크랭크 암 길이(페달 중심까지) */
  crankRadius: number;
  /** 안장 윗면 중앙 */
  saddleTop: [number, number];
  /** 손 위치 후보: hoods(기본·스퍼트), tops(오르막), drops(내리막 에어로). 플랫바는 셋이 같습니다. */
  hands: { hoods: [number, number]; tops: [number, number]; drops: [number, number] };
  handlebar: 'drop' | 'flat';
};
...
type BikeGeometry = {
  ...
  saddleX: number;
  saddleY: number;
  barAnchor: [number, number];
  handlebar: 'drop' | 'flat';
  forkThickness: number;
};

function bikeGeometry(category: BikeCategory): BikeGeometry {
  if (category === 'minivelo') {
    ...
    return {
      points,
      wheel: { radius: 6, tireThickness: 1, spokeStep: 60 },
      saddleX: 22, saddleY: 8,
      barAnchor: [points.headTop[0], points.headTop[1] + 1],
      handlebar: 'flat',
      forkThickness: 2,
    };
  }
  ...
  return {
    ...
    saddleX: isMtb ? 21 : 22, saddleY: 8,
    barAnchor: points.headTop,
    handlebar: category === 'road' || category === 'gravel' ? 'drop' : 'flat',
    forkThickness: isMtb ? 3 : 2,
  };
}

const CHAINRING_RADIUS = 3;
const CRANK_RADIUS = 3.5;

export function bikeRig(category: BikeCategory): BikeRig {
  const geometry = bikeGeometry(category);
  const [hx, hy] = geometry.barAnchor;
  // 드롭바: 톱 구간 (hx+2..hx+7, hy-4), 후크 (hx+8, hy-3..hy-1) → 후드=톱 앞끝, 드롭=후크 중간
  // 플랫바: 바 (hx-3..hx+6, hy-4), 앞쪽 그립 (hx+5..hx+6) → 손은 앞 그립 위 한 자리
  const hands: BikeRig['hands'] = geometry.handlebar === 'drop'
    ? { hoods: [hx + 7, hy - 4], tops: [hx + 4, hy - 4], drops: [hx + 8, hy - 2] }
    : { hoods: [hx + 5, hy - 4], tops: [hx + 5, hy - 4], drops: [hx + 5, hy - 4] };
  return {
    category,
    gridWidth: GRID_W,
    gridHeight: GRID_H,
    axleY: AXLE_Y[category],
    rear: geometry.points.rear,
    front: geometry.points.front,
    bb: geometry.points.bb,
    wheel: { ...geometry.wheel },
    chainringRadius: CHAINRING_RADIUS,
    crankRadius: CRANK_RADIUS,
    saddleTop: [geometry.saddleX, geometry.saddleY],
    hands,
    handlebar: geometry.handlebar,
  };
}

// 다이아몬드 프레임 + 구동계 + 싯포스트·안장 공통 플로팅
function plotDiamondFrame(grid: BikeGrid, points: FramePoints, options: { forkThickness?: number; stayThickness?: number } = {}) {
...
function plotDrivetrain(grid: BikeGrid, bb: [number, number], rear: [number, number], chainringRadius: number, detach: Detach) {
  ...
  // 크랭크 암과 페달 (측면 뷰에서는 앞쪽 크랭크만 강조). 레이스 연출에서는 회전 레이어가 대신 그립니다.
  if (detach.crankset) return;
  plotLine(grid, bb[0], bb[1], bb[0] + 4, bb[1] + 3, 'crank', 2);
  plotRect(grid, bb[0] + 3, bb[1] + 4, bb[0] + 6, bb[1] + 4, 'pedal');
}

function plotSaddle(grid: BikeGrid, seat: [number, number], saddleX: number, saddleY: number) {
...
function plotDropBar(grid: BikeGrid, headTop: [number, number]) {
  const [hx, hy] = headTop;
...
// 플랫바: 수평 바 + 양끝 그립
function plotFlatBar(grid: BikeGrid, headTop: [number, number]) {
  const [hx, hy] = headTop;
...
// 카테고리(+분리 부품 조합)별 그리드 캐시. 색과 무관한 순수 지오메트리입니다.
const gridCache = new Map<string, BikeGrid>();

function detachFrom(parts: BikeDetachablePart[] | undefined): Detach {
  if (!parts || parts.length === 0) return NO_DETACH;
  return { spokes: parts.includes('spokes'), crankset: parts.includes('crankset') };
}

/**
 * 카테고리별 자전거 그리드를 만듭니다(캐시). detach에 든 부품은 그리지 않아
 * 레이스 연출이 회전 레이어를 따로 얹을 수 있습니다.
 */
export function buildBikeGrid(category: BikeCategory, options: { detach?: BikeDetachablePart[] } = {}): BikeGrid {
  const detach = detachFrom(options.detach);
  const key = `${category}|${detach.spokes ? 's' : ''}${detach.crankset ? 'c' : ''}`;
  const cached = gridCache.get(key);
  if (cached) return cached;
  const grid = makeGrid();
  const geometry = bikeGeometry(category);
  const { points, wheel } = geometry;

  plotWheel(grid, points.rear[0], points.rear[1], wheel, detach);
  plotWheel(grid, points.front[0], points.front[1], wheel, detach);
  plotDrivetrain(grid, points.bb, points.rear, CHAINRING_RADIUS, detach);
  plotDiamondFrame(grid, points, { forkThickness: geometry.forkThickness });

  if (category === 'minivelo') {
    plotSaddle(grid, points.seat, geometry.saddleX, geometry.saddleY);
    plotFlatBar(grid, geometry.barAnchor);
    ...
    plotSaddle(grid, points.seat, geometry.saddleX, geometry.saddleY);
    if (geometry.handlebar === 'drop') plotDropBar(grid, geometry.barAnchor);
    else plotFlatBar(grid, geometry.barAnchor);
...
function renderGridToGraphics(
  ...
  originY: number,
  cell: number,
  colorFor: (drawn: BikeCell) => number,
  alphaFor: (drawn: BikeCell) => number,
) {
  forEachRun(
    grid,
    (drawn) => `${colorFor(drawn)}:${alphaFor(drawn)}`,
    (x, y, length, drawn) => {
      g.fillStyle(colorFor(drawn), alphaFor(drawn));
      g.fillRect(originX + x * cell, originY + y * cell, length * cell, cell);
    },
  );
}

/** 실루엣·컬러웨이 옵션에 따른 셀 색상 결정 (순수 함수) */
export function resolveBikeCellColor(options: Pick<PixelBikeOptions, 'silhouette'>, colorway: BikeColorway) {
  return (drawn: BikeCell) => (options.silhouette
...
export function drawPixelBike(
  ...
  renderGridToGraphics(
    ...
    resolveBikeCellColor(options, colorway),
...
export function addPixelBikeImage(
  ...
  if (!scene.textures.exists(key)) {
    ...
    renderGridToGraphics(g, grid, 0, 0, cell, resolveBikeCellColor(options, colorway), () => 1);
...
const iconCache = new Map<BikePartIconType, BikeGrid>();

function buildPartIconGrid(type: BikePartIconType): BikeGrid {
  const cached = iconCache.get(type);
  if (cached) return cached;
  const grid = makePixelGrid<BikeCell>(ICON_W, ICON_H);
  if (type === 'frame') {
...
// cellAt은 테스트·라이더 스프라이트에서 그리드 셀 조회에 쓰도록 재노출합니다.
export { cellAt as bikeCellAt };
